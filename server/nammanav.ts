...
type Candidate = { name: string; category: string; rating: number | null; reviews: number | null; open: string; cost: string; distance: number | null; confidence: number; source: string; url: string | null; reasons: string[]; warnings: string[]; claims: Evidence[] };
...
export const recommend = publicProcedure.input(requestSchema).mutation(async ({ input }) => {
  ...
  if (live) {
    try {
      const providerCall = async (engine: "google_maps" | "google" | "google_news", purpose: string, query: string): Promise<SearchRecord[]> => {
      ...
      const records = await providerCall("google_maps", "local discovery", `${sanitizedText} near ${sanitizedLocation}`);
      ...
      try { verificationResults = await providerCall("google", "official information verification", `official information and amenities for ${sanitizedText} near ${sanitizedLocation}`); engineResultCounts.google = verificationResults.length; } catch (error) { providerError = error instanceof Error ? error.message : "PROVIDER_ERROR"; }
      try { challengeResults = await providerCall("google", "challenge recommendation", `problems complaints closure poor Wi-Fi charging reviews for ${sanitizedText} near ${sanitizedLocation}`); } catch (error) { providerError = error instanceof Error ? error.message : "PROVIDER_ERROR"; }
  ...
  const tasks = [{ engine: "google_maps", purpose: "local discovery", query: `${sanitizedText} near ${sanitizedLocation}` }, { engine: "google", purpose: "official information verification", query: `official information for ${sanitizedLocation}` }, { engine: "google", purpose: "challenge recommendation", query: `problems complaints closure poor Wi-Fi charging reviews for ${sanitizedText} near ${sanitizedLocation}` }, ...(input.currentCheck ? [{ engine: "google_news", purpose: "current disruption checking", query: `${sanitizedLocation} closure disruption safety` }] : [])];
  ...
  const replay = ["Privacy sanitization", "Constraint discovery", "Query planning", "Maps discovery", "Official verification", "Challenge search", ...(input.currentCheck ? ["News change check"] : []), "Hard filtering", "Confidence scoring", "Explainable ranking", "Action plan"];
  ...
  const challengePenalty = challengeResults.length ? Math.min(12, challengeResults.length * 2) : 0;
  const finalRecommendations = recommendations.map((item, index) => ({ ...item, score: Math.max(0, item.score - (index === 0 ? challengePenalty : 0)) }));
  const maxRequests = 5;
  ...
  const dossier = { generatedAt: new Date().toISOString(), constraints, privacy: { ...({ enabled: input.privacyMode, rawRequestStored: false, sentContext: sanitizedText, broadLocation: sanitizedLocation, blocked: input.privacyMode ? ["email", "phone", "exact address"] : [] }) }, tasks, telemetry: telemetrySummary, providerTelemetry: telemetry, recommendations: finalRecommendations, initialRecommendation, scoreChange: initialRecommendation ? finalRecommendations[0].score - initialRecommendation.score : 0, challengeResults: challengeResults.map((record) => ({ title: typeof record.title === "string" ? record.title : "Untitled challenge result", snippet: safeSnippet(record), url: typeof record.link === "string" ? record.link : null })), evidence: graph, uncertainty: uncertain, trust, limitations: ["Information can change. Confirm important details before travelling.", "Distance may be unavailable from provider results."] };
  return { mode, providerError, constraints, privacy: { enabled: input.privacyMode, rawRequestStored: false, sentContext: sanitizedText, broadLocation: sanitizedLocation, blocked: input.privacyMode ? ["email", "phone", "exact address"] : [] }, workflow: replay, tasks, telemetry: telemetrySummary, providerTelemetry: telemetry, dossier, trust, mission: { engines: telemetry.length ? telemetry.map((entry) => ({ engine: entry.engine, purpose: entry.purpose, status: entry.status, resultCount: entry.resultCount, responseTimeMs: entry.latencyMs, contribution: entry.purpose })) : tasks.map((task) => ({ engine: task.engine, purpose: task.purpose, status: "not called", resultCount: 0, responseTimeMs: 0, contribution: task.purpose })), requestCount: live ? telemetry.length : 0, claimsCreated: graph.length, claimsChallenged: graph.filter((edge) => edge.polarity === "challenges" || edge.polarity === "conflicting").length, candidatesEliminated: rejected.length, mode }, recommendations: finalRecommendations, initialRecommendation, scoreChange: initialRecommendation ? finalRecommendations[0].score - initialRecommendation.score : 0, rejected, uncertain, evidenceGraph: graph, freshnessRadar: freshness, changeRadar: input.currentCheck ? { status: providerError && engineResultCounts.google_news === 0 ? "unavailable" : "checked", warning: providerError && engineResultCounts.google_news === 0 ? "Current News check unavailable" : newsResults.length ? "News signal detected; confirm before travel." : "No current disruption signal returned; confirm before travel." } : null, replay, confidenceMeter: recommendations.map((item) => ({ name: item.name, sourceQuality: Math.round(item.confidence * 100), freshness: 96, agreement: 78, completeness: item.claims.length ? 80 : 20, overall: Math.round(item.confidence * 100 * 0.3 + 96 * 0.2 + 78 * 0.2 + 80 * 0.3) })), actionPlan: eligible[0] ? { recommendation: eligible[0].name, confirmBeforeLeaving: ["opening status", "Wi-Fi and charging availability", "current price"], fallback: eligible[1]?.name ?? "No fallback found", confidence: eligible[0].confidence, limitations: ["Information can change. Confirm important details before travelling."] } : { recommendation: null, confirmBeforeLeaving: [], fallback: "No eligible candidate", confidence: 0, limitations: ["No eligible result was found."] }, noMatch };
});
