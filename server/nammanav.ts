...
type LiveSearchResult = { records: SearchRecord[]; httpStatus: number };
class ProviderHttpError extends Error { constructor(message: string, public readonly httpStatus: number) { super(message); } }
...
async function liveSearch(engine: "google_maps" | "google" | "google_news" | "google_maps_reviews", query: string): Promise<LiveSearchResult> {
  ...
  const timeoutSeconds = Math.max(2, Math.min(12, Number(process.env.SERPAPI_TIMEOUT_SECONDS ?? 12)));
  const response = await fetch(url, { signal: AbortSignal.timeout(timeoutSeconds * 1000) });
  if (response.status === 402 || response.status === 429) throw new ProviderHttpError("PROVIDER_QUOTA", response.status);
  if (!response.ok) throw new ProviderHttpError("PROVIDER_ERROR", response.status);
  ...
  return { records: Array.isArray(records) ? records.slice(0, 8) : [], httpStatus: response.status };
}
...
export const recommend = publicProcedure.input(requestSchema).mutation(async ({ input }) => {
  ...
  const telemetry: Array<{ engine: string; purpose: string; query: string; status: string; httpStatus: number | null; latencyMs: number; error: string | null; retries: number; timedOut: boolean; resultCount: number }> = [];
  if (live) {
    try {
      const providerCall = async (engine: "google_maps" | "google" | "google_news" | "google_maps_reviews", purpose: string, query: string): Promise<SearchRecord[]> => {
        ...
        try {
          const result = await liveSearch(engine, query);
          telemetry.push({ engine, purpose, query, status: "complete", httpStatus: result.httpStatus, latencyMs: Date.now() - started, error: null, retries: 0, timedOut: false, resultCount: result.records.length });
          rawSerpApiResponse[engine] = result.records;
          return result.records;
        } catch (error) {
          const message = error instanceof Error ? error.message : "PROVIDER_ERROR";
          const httpStatus = error instanceof ProviderHttpError ? error.httpStatus : null;
          telemetry.push({ engine, purpose, query, status: message === "PROVIDER_QUOTA" ? "quota" : "error", httpStatus, latencyMs: Date.now() - started, error: message, retries: 0, timedOut: /timeout|aborted/i.test(message), resultCount: 0 });
          throw error;
        }
      };
      ...
      const initialFailure = initialResults.slice(0, 3).find((item) => item.status === "rejected");
  ...
  const graph = recommendations.flatMap((item) => item.claims.map((evidence, claimIndex) => ({ recommendation: item.name, claimId: `${evidence.claimId}:${item.name}:${claimIndex}`, claim: evidence.field, value: evidence.value, sourceType: evidence.source, sourceUrl: evidence.url, confidence: evidence.confidence, freshnessHours: evidence.ageHours, status: evidence.status, polarity: evidence.polarity, scoreImpact: evidence.scoreImpact, decisionImpact: evidence.scoreImpact < 0 ? "Reduced readiness" : evidence.scoreImpact > 0 ? "Increased readiness" : "No direct score change", snippet: evidence.snippet, engine: evidence.engine, retrievedAt: evidence.retrievedAt, supports: evidence.supports })));
  ...
  const challengeRecords = [...challengeResults, ...reviewResults, ...newsResults];
  const counterEvidenceRecords = challengeRecords.filter((record) => /closed|closure|complaint|poor|bad|unavailable|not available|mismatch|wrong branch|missing|no wifi|no wi-fi|overpriced|relocat|disruption|unsafe|crowded|temporarily/i.test(`${record.title ?? ""} ${record.snippet ?? ""} ${record.description ?? ""} ${record.content ?? ""}`));
  const conflictingClaimCount = graph.filter((edge) => edge.polarity === "conflicting").length;
  const challengePenalty = Math.min(15, conflictingClaimCount * 8 + counterEvidenceRecords.length * 4);
  ...
  const challengeSummary = challengeResults.length || reviewResults.length || newsResults.length || conflictingClaimCount ? "Targeted challenge evidence returned; inspect each source before acting." : "No contradiction found in the searched evidence.";
  const finalTop = finalRecommendations[0] ?? null;
  const recommendationChanged = Boolean(initialRecommendation && finalTop && initialRecommendation.name !== finalTop.name);
  const decisionMutationLog = [
    { stage: "Initial score", detail: initialRecommendation ? `${initialRecommendation.name} · ${initialRecommendation.score}` : "No eligible initial recommendation" },
    { stage: "Evidence discovered", detail: counterEvidenceRecords.length ? `${counterEvidenceRecords.length} counter-evidence signal${counterEvidenceRecords.length === 1 ? "" : "s"} returned by the challenge search` : conflictingClaimCount ? `${conflictingClaimCount} conflicting source signal${conflictingClaimCount === 1 ? "" : "s"} detected` : "No direct counter-evidence signal was found" },
    { stage: "Score impact", detail: challengePenalty ? `-${challengePenalty} challenge penalty applied to the leading candidate` : "0 · no counter-evidence penalty applied" },
    { stage: "Challenge result", detail: challengeSummary },
    { stage: "Final score", detail: finalTop ? `${finalTop.name} · ${finalTop.score}` : "No safe match found" },
    { stage: "Decision impact", detail: recommendationChanged ? `Recommendation changed to ${finalTop?.name}` : "Recommendation unchanged; uncertainty remains visible" },
  ];
  ...
  const maxRequests = Math.max(1, Math.min(10, Number(process.env.SERPAPI_MAX_REQUESTS_PER_WORKFLOW ?? 5)));
  providerError = telemetry.find((entry) => (entry.status === "error" || entry.status === "quota") && entry.engine !== "google_news" && entry.engine !== "google_maps_reviews")?.error ?? (telemetry.some((entry) => entry.status === "quota" && entry.engine !== "google_news" && entry.engine !== "google_maps_reviews") ? "PROVIDER_QUOTA" : null);
  ...
  const providerHealth = { status: !live ? "mock" : telemetry.some((entry) => entry.status === "quota") ? "quota-limited" : telemetry.some((entry) => entry.error) ? "degraded" : telemetry.length ? "healthy" : "unavailable", credentialConfigured: live ? Boolean(process.env.SERPAPI_KEY) : false, requestsUsed: telemetrySummary.requestsUsed, remainingWorkflowAllowance: telemetrySummary.remainingWorkflowAllowance, maxRequestsPerWorkflow: maxRequests, engines: telemetry.map((entry) => ({ engine: entry.engine, purpose: entry.purpose, status: entry.status, httpStatus: entry.httpStatus, latencyMs: entry.latencyMs, resultCount: entry.resultCount, error: entry.error })) };
  ...
  const dossier = { receiptId: `nammanav-${Date.now()}`, decisionContract, verdict, execution, auditStrictness: input.auditStrictness, generatedAt: new Date().toISOString(), attackSurface, temporalTruth, conflicts, stressTests, challengeSummary, constraints, privacy: { ...({ enabled: input.privacyMode, rawRequestStored: false, sentContext: sanitizedText, broadLocation: sanitizedLocation, blocked: input.privacyMode ? ["email", "phone", "exact address"] : [] }) }, tasks, telemetry: telemetrySummary, providerHealth, providerTelemetry: telemetry, recommendations: finalRecommendations, initialRecommendation, scoreChange: initialRecommendation ? finalRecommendations[0].score - initialRecommendation.score : 0, decisionMutationLog, challengeResults: [...challengeResults, ...reviewResults, ...newsResults].map((record) => ({ title: typeof record.title === "string" ? record.title : "Untitled challenge result", snippet: safeSnippet(record), url: typeof record.link === "string" ? record.link : null })), evidence: graph, uncertainty: uncertain, trust, limitations: ["Information can change. Confirm important details before travelling.", "Distance may be unavailable from provider results."] };
  return { mode, providerError, decisionContract, decisionMutationLog, verdict, execution, auditStrictness: input.auditStrictness, attackSurface, temporalTruth, conflicts, stressTests, challengeSummary, constraints, privacy: { enabled: input.privacyMode, rawRequestStored: false, sentContext: sanitizedText, broadLocation: sanitizedLocation, blocked: input.privacyMode ? ["email", "phone", "exact address"] : [] }, workflow: replay, tasks, telemetry: telemetrySummary, providerHealth, providerTelemetry: telemetry, dossier, trust, mission: { engines: telemetry.map((entry) => ({ engine: entry.engine, purpose: entry.purpose, status: entry.status, httpStatus: entry.httpStatus, resultCount: entry.resultCount, responseTimeMs: entry.latencyMs, contribution: entry.purpose })), requestCount: live ? telemetry.length : 0, claimsCreated: graph.length, claimsChallenged: graph.filter((edge) => edge.polarity === "challenges" || edge.polarity === "conflicting").length, candidatesEliminated: rejected.length, mode }, recommendations: finalRecommendations, initialRecommendation, scoreChange: initialRecommendation ? finalRecommendations[0].score - initialRecommendation.score : 0, rejected, uncertain, evidenceGraph: graph, freshnessRadar: freshness, changeRadar: input.currentCheck ? { status: providerError && engineResultCounts.google_news === 0 ? "unavailable" : "checked", warning: providerError && engineResultCounts.google_news === 0 ? "Current News check unavailable" : newsResults.length ? "News signal detected; confirm before travel." : "No current disruption signal returned; confirm before travel." } : null, replay, confidenceMeter: recommendations.map((item) => ({ name: item.name, sourceQuality: Math.round(item.confidence * 100), freshness: 96, agreement: 78, completeness: item.claims.length ? 80 : 20, overall: Math.round(item.confidence * 100 * 0.3 + 96 * 0.2 + 78 * 0.2 + 80 * 0.3) })), actionPlan: finalRecommendations[0] ? { recommendation: finalRecommendations[0].name, confirmBeforeLeaving: ["opening status", "Wi-Fi and charging availability", "current price"], fallback: finalRecommendations[1]?.name ?? "No fallback found", confidence: finalRecommendations[0].confidence, limitations: ["Information can change. Confirm important details before travelling."] } : { recommendation: null, confirmBeforeLeaving: [], fallback: "No eligible candidate", confidence: 0, limitations: ["No eligible result was found."] }, noMatch };
});
