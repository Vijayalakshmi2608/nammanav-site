...
type Evidence = { field: string; value: string | number | boolean; source: string; url: string | null; confidence: number; ageHours: number; status: string; snippet: string | null; engine: string; retrievedAt: string; supports: boolean };
...
const claim = (field: string, value: string | number | boolean, source: string, url: string | null, confidence: number, status = "verified", extras: Partial<Evidence> = {}): Evidence => ({ field, value, source, url, confidence, ageHours: 1, status, snippet: null, engine: source === "Maps" ? "google_maps" : source === "News" ? "google_news" : "google", retrievedAt: new Date().toISOString(), supports: status !== "conflicting" && status !== "stale", ...extras });
...
export const recommend = publicProcedure.input(requestSchema).mutation(async ({ input }) => {
  ...
  const graph = recommendations.flatMap((item) => item.claims.map((evidence) => ({ recommendation: item.name, claim: evidence.field, value: evidence.value, sourceType: evidence.source, sourceUrl: evidence.url, confidence: evidence.confidence, freshnessHours: evidence.ageHours, status: evidence.status, snippet: evidence.snippet, engine: evidence.engine, retrievedAt: evidence.retrievedAt, supports: evidence.supports })));
